...
main()
{
  // This protocol has been adapted from Applied Biosystems(TM) Automated DNA
  // Sequencing Guide protocol using BigDye Primer.
  ...
  tap(rxn_tube);
  comment("NOTE:4 different reactions will have to be set up for the four "
          "different bases-A, G, T and C.");
  ...
  // centrifuge the sample for 20 mins at SPEED_MAX and room temperature to
  // obtain the pellet
  next_step();
  ...
  // centrifuge the sample for 10 mins at SPEED_MAX and room temperature to
  // obtain the pellet
  next_step();
  ...
  // preparing dye primer reaction products for electrophoresis(combining the
  // four products)
  next_step("Sequencing and detection");
  // ethanol precipitation
  measure_fluid(eth95, vol(1, ML), rxn_tube);
  comment("NOTE: combine all the 4 products (A, G, T and C) from the "
          "preparation step.");
  dissolve(rxn_tube);
  store_for(rxn_tube, RT, time(10, MINS));

  // centrifugation at SPEED_MAX for 15 mins at room temperature to obtain the
  // pellet
  next_step();
  ...
  // addition of template suppression reagent and heating for 2 minutes at 95deg
  // C
  next_step();
