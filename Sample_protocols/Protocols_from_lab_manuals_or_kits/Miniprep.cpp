...
main()
{
  ...
  Fluid medium = new_fluid(
    "rich medium (LB, YT, or Terrific medium)containing appropriate antibiotic",
    vol(2, ML));
  Fluid sol1 =
    new_fluid("Alkaline Lysis Solution I",
              "50 mM Glucose,25 mM Tris-HCl (pH 8.0), 10 mM EDTA (pH 8.0)");
  Fluid sol2 = new_fluid("freshly prepared Alkaline Lysis Solution II",
                         "0.2 N NaOH, 1% SDS (w/v)");
  Fluid sol3 = new_fluid("Alkaline Lysis Solution III",
                         "5 M sodium acetate, glacial acetic acid");
  Fluid ethanol = new_fluid("ethanol at room temperature");
  Fluid phe_chloro = new_fluid("phenol:chloroform", "1:1, v/v");
  Fluid te =
    new_fluid("TE containing 20 �g/ml DNase-free RNase (pancreatic RNase)");
  ...
  next_step();
  centrifuge_phases_top(microfuge_tube,
                        speed(SPEED_MAX, RPM),
                        4,
                        time(5, MINS),
                        sterile_microfuge_tube1);
  ...
  next_step();
  centrifuge_pellet(
    sterile_microfuge_tube2, speed(SPEED_MAX, RPM), 4, time(5, MINS));
  ...
  centrifuge_pellet(
    sterile_microfuge_tube2, speed(SPEED_MAX, RPM), 4, time(2, MINS));
  comment("Take care with this step, as the pellet sometimes does not adhere "
          "tightly to the tube.");
  ...
  store_until(
    sterile_microfuge_tube2, RT, ETHANOL_EVAP, time_range(5, 10, MINS));
