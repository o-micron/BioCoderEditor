...
// from the paper "SIRT1 Redistribution on Chromatin Promotes Genomic Stability
// but Alters Gene Expression during Aging" -Sinclair et al.,
//DOI 10.1016/j.cell.2008.10.025
Fluid transfect_dna, nam, pbs, chloroform, isopropanol, ethanol, depc_water,
  rt_primer, rttaq, rtpcr_buffer, te;
...
main()
{
  ...
  Fluid transfect_dna =
    new_fluid("DNA to be transfected", "pCBASce/pCAGGS-Dsred");
  ...
  next_step();
  centrifuge_pellet(
    sterile_microfuge_tube, speed(SPEED_MAX, RPM), RT, time(2, MINS));
  ...
  cell_culture(cell_culture_chamber,
               es_med,
               SPEED_MAX,
               RT,
               3 * 24 * 60 * 60,
  ...
  // transfection of cultured cells with pCBASce/pCAGGS-Dsred in the presence of
  // Fugene 6 transfection agent
  next_step();
  ...
  // cross-linking of cultured cells with 1% fromaldehyde and then quenching
  // with glycine
  ...
  centrifuge_phases_top(sterile_microfuge_tube1,
                        speed(SPEED_MAX, RPM),
                        4,
                        time(15, MINS),
  ...
  next_step();
  centrifuge_pellet(
    sterile_microfuge_tube2, speed(SPEED_MAX, RPM), 4, time(10, MINS));
  ...
  thermocycler(
    sterile_microfuge_tube2, 15, 95, time(15, SECS), 60, time(60, SECS));
