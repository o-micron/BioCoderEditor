...
// paper "Transcriptional Instability is Not a Universal Attribute of
// Aging"-Quake et al., Department of Bioengineering, Stanford University.
int
main()
{
  ...
  Fluid lysis_buffer = new_fluid(
    "ACK lysis buffer", "150 mM NH4Cl, 10 mM KHCO3, 0.1 mM EDTA, pH 7.2");
  ...
  // This is followed by staining of remaining cells and HSCs with
  // fluorescence-conjugated antibodies
  next_step("Fluorescence staining:");
  measure_fluid(staining_buffer, vol(200, UL), rxn_tube);
  vortex(rxn_tube);

  // centrifugation, discarding the supernatant and resuspension of cells in
  // staining buffer
  next_step();
