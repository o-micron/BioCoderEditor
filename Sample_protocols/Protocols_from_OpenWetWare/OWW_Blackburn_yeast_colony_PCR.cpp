...
main()
{
  ...
  Fluid f_primer = new_fluid("Forward primer", "100�M");
  ...
  //   2. Using a sterile pipette tip, pick a small colony and resuspend in
  //   NaOH.
  ...
  //   3. Boil the samples on a PCR machine by incubating the tubes at 99C for
  //   10 minutes.
  //          * In the mean time, prepare the master mix for the PCR reaction.
  //          * The boiled samples are stable at room temp for some time. Keep
  //          on ice or freeze for longer.
  next_sub_step();
  thermocycler(pcr_tube1, 99, time(10, MINS));
  comment("In the mean time, prepare the master mix for the PCR reaction.");
  comment("The boiled samples are stable at room temp for some time. Keep on "
          "ice or freeze for longer.");
  ...
  next_step("PCR");
  first_sub_step();
  Fluid fluid_array[7] = { q_solution, pcr_buffer, dntp, f_primer,
                           r_primer,   taq,        water };
  Volume* volumes[7] = { vol(2, UL),   vol(1, UL),   vol(0.2, UL), vol(0.2, UL),
                         vol(0.2, UL), vol(0.1, UL), vol(5.3, UL) };
  ...
  //   3. Transfer 1uL of boiled samples to the master mix aliquots (a
  //   multi-channel pipette is helpful here).
  next_sub_step();
